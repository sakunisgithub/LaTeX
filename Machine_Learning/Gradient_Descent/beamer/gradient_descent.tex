...
\author[Ananda Biswas]{Ananda Biswas \\[0.5em] \href{mailto:ami.ananda@bhu.ac.in}{\faEnvelope \, ami.ananda@bhu.ac.in}}
\date{}
...
\begin{tikzpicture}[
    ...
    % Nodes
    \only<1->{\node[box] (head) {Machine Learning};}
    \only<2->{\node[box, right of=head] (n1) {Objective $\mathscr{L}(\symbf{\theta})$};}
...
\]
	...
	$$\mathscr{L}(w, b) = \sum \limits_{i = 1}^{n} \left(y_i - f(x_i)\right)^2$$
	\end{block}
...
\only<4->{$\mathscr{L}(w, b) = \dfrac{1}{2} \sum \limits_{i = 1}^{n} \left(y_i - f(x_i)\right)^2$ will tell us.}
...
\begin{column}{0.5\textwidth}
\begin{align*}
\mathscr{L}(w, b) &= \dfrac{1}{2} \sum \limits_{i = 1}^{n} \left(y_i - f(x_i)\right)^2 \\[0.5em]
...
$$\mathcal{F}({x}) = \mathcal{F}({a}) + ({x} - {a}) \mathcal{F'}({a}) + \dfrac{1}{2!} ({x} - {a})^2 {\mathcal{F}''}({a}) + \cdots$$
\end{block}
}
...
\begin{figure}[!htbp]
\centering
\includegraphics[scale=0.24]{cat_side_eye}
\end{figure}
\end{frame}
...
\only<4>{\centering \includegraphics[scale=0.35]{Contour_3}}
\only<5>{\centering \includegraphics[scale=0.35]{Contour_4}}
\only<6>{\centering \includegraphics[scale=0.35]{Contour_5}}
\end{frame}
...
\section{Nesterov Gradient Descent}
